...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // the goal location is the stop point, 
      // we need to put the goal at the point which behind the stop point at _stop_line_buffer
      // it is why we plus M_PI
      auto ang = goal.rotation.yaw + M_PI;
      goal.location.x += this->_stop_line_buffer*std::cos(ang);  // <- Fix This
      goal.location.y += this->_stop_line_buffer*std::sin(ang);  // <- Fix This
      ...
      // at stop point the velocity should be 0
      ...
      // speed calculation by nominal state
      goal.velocity.x = this->_speed_limit*std::cos(goal.rotation.yaw);  // <- Fix This
      goal.velocity.y = this->_speed_limit*std::sin(goal.rotation.yaw);  // <- Fix This
      goal.velocity.z = 0;
    }
